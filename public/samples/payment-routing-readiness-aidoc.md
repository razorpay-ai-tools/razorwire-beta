# AIDoc: Payment Routing Readiness Explainer
...
Readiness should check both a lightweight ping and pool starvation indicators. A pod with a fully saturated connection pool should not continue to accept new traffic.

### Redis

Redis readiness is required for paths that depend on idempotency, caching, or compliance state. The check should be conditional when Redis is not part of a pod's critical request path.

### Kafka

Kafka producer readiness is required when the API pod must publish events as part of the request lifecycle. A failed producer should remove the pod from traffic rather than silently creating event gaps.

### gRPC clients

Critical downstream gRPC clients should expose connectivity and in-flight pressure signals. If client concurrency is exhausted, readiness should fail before user traffic piles up.
...
- What thresholds should define database and gRPC pool starvation?
- Which Redis-backed flows are critical for each pod?
- Should Kafka readiness block all traffic or only specific routes?
- How should readiness failures be surfaced in dashboards and alerts?
