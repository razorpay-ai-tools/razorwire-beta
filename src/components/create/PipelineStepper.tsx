...
interface PipelineStepperProps {
  jobId: string;
  /**
   * Fires once, on the terminal state. On `published` the post already exists and its id
   * is carried on `job.postId`.
   */
  onDone: (job: Job) => void;
...
export function PipelineStepper({ jobId, onDone, onRetry }: PipelineStepperProps) {
  ...
  useEffect(() => {
    ...
    async function tick(): Promise<void> {
      try {
        ...
        // The pipeline creates the post itself — with the rendered MP4 on it, when one was
        // asked for — and hands back its id. Creating a second post from `job.storyboard`
        // here is what made a finished video open as a browser reel: the duplicate carried
        // the same scenes and no `mediaUrl`, and this is the id the panel navigated to.
        if (next.state === 'published' && !next.postId) {
          setPublishError('The job finished but produced no post, so there is nothing to open.');
          return;
        }
  ...
  const settled = activeKey === 'published' && !publishError;
  /** Nothing is in flight any more: the job failed, or it published nothing openable. */
